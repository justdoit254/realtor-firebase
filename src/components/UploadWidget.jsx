...
const UploadWidget = ({ children, onUpload }) => {
  ...
  const createWidget = () => {
    ...
    }; //up to 1.5MB images and 10MB video allowed

    return cloudinary?.createUploadWidget(options, (error, result) => {
      // The callback is a bit more chatty than failed or success so only trigger when one of those are the case.
      // You can additionally create a separate handler such as onEvent and trigger it on ever occurrence
